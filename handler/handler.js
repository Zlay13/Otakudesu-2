...
const completeAnimeHandler = async (req, res) => {
  ...
  let data;
  try {
    data = await otakudesu.getCompleteAnimeData(page);
  } catch (e) {
    console.log(e);
    return res.status(500).json({
      status: 'Error',
...
export default {
  searchAnimeHandler, singleAnimeHandler, episodeHandler, homeHandler, ongoingAnimeHandler, completeAnimeHandler
};
